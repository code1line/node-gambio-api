...
class Validator {
  ...
  static checkArray(array, type) {
    ...
    const entityName = check.assigned(type) ? type : 'Array';
   ...
   * @static
   */
  static checkTypedArray(typedArray, expectedType, type) {
    // Valid expected type values.
    const validExpectedTypes = ['string', 'integer', 'object', 'number'];

    // Check entity type for error message.
    this._checkTypeArgument(type);

    // Assign typed array entity name.
    const entityName = check.assigned(type) ? type : 'Typed array';
