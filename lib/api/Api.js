...
class Api {
  ...
  /**
   * Converts a provided array of fieldnames limit directives to URL parameters.
   *
   * @param {Array} limits Fieldnames.
   * @return {String}
   * @throws ArgumentError If a fieldname value is not a string.
   * @private
   */
  _convertFieldLimitArray(limits) {
    ...
    for (const fieldName of limits) {
      Validator.checkString(fieldName);
    }

    // Generate comma-separated fieldname va
  }
}
