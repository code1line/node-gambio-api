...
class Api {
   ...
   * @private
   */
  _modifyUrlByManipulatorArguments(url, sorting, limits) {
    ...
    // Append '?' to URL if any parameter ís set.
    if (isSetSorting || isSetLimits) {
      modifiedUrl += '?';
    }

    // Append sorting parameters to URL, if provided.
    if (isSetSorting) {
      modifiedUrl += this._convertSortingObject(sorting);
    }

    // Append ampersand and limiting field names parameters to URL, if provided.
    if (isSetLimits) {
      modifiedUrl += `&${this._convertFieldLimitArray(limits)}`;
    }
